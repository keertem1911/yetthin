...
public class GroupServiceImp extends BaseService implements GroupService, ValueFormatUtil {
	...
	public String getDetail(String groupId, String path,String userId) {
		...
		double totleIncome= group.getGroupIncomeTotle();
		...
		if (format_double_1.format((group.getGroupIncomeTotle() - group.getGroupInitMoney())).equals("0") == false) {
			detail.setTotleIncome(
					format_double_3.format( totleIncome/ 10000));
			detail.setTotleIncomeRatio(format_double_3.format(
					100*( totleIncome/ group.getGroupInitMoney())));
		} else {
		...
		if (createProfit == null) {
			...
			if (createPrice == 0) {
				...
				if (group.getGroupInitMoney() != 0){
					detail.setNetIncome(format_double_3.format((group.getGroupInitMoney()+totleIncome) / group.getGroupInitMoney()));
					System.out.println(createPrice+","+group.getGroupInitMoney());
				}
				else
					detail.setNetIncome("0");
			}
		...
		if (shareList != null) {
			...
			double useMoney= group.getGroupInitMoney() - share_Start_Currnt.get("start_fund");
			detail.setFreeFund(Double.toString(useMoney));
								...
								+ "\"lastPrice\":\""+subStock[0]+"\",\"upOrDownPrice\":\""+subStock[2]+"\","
										+ "\"increase\":\""+plus+"\","
												+ "\"incomeRadio\":\""+format_double_3.format(shareIncomeS)+"\","
														+ "\"number\":\""+radioMap.get(string).get().getShareCurrentNum()+"\"}");
				if (i < key.size() - 1)
					buffer.append(",");
				i++;
			}
		...
		for (int i = 0; i < jsonarray.size(); ++i) {
			...
			if (values != null)
				if (s.getShareStocklableId().split(SPLIT_STR).length > 0
					...
					String number = format_double_3.format(Math.floor(fund/lastPrice/100));
					fund=Math.floor(fund/lastPrice/100)*lastPrice;
					s.setShareStartFund(fund);
					s.setShareCurrentIncome(fund);
...
//			List<BarDataOfDay> listAvg = barDataOfDayMapper.selectByDates("2016-12-01","2016-12-10");
		...
		List<String> dates=  barDataOfDayMapper.selectLimitDay(4);
		...
		barDataOfDays =strategyFuntion.strategy(barDataOfDays, type, limit);
		if(barDataOfDays.size()!=0){
		Map<String, Boolean> filterBoolean =new HashMap<>();
		lists.forEach(stocklable->{
			filterBoolean.put(stocklable.getStocklableCode(), false);
		});
		barDataOfDays.forEach(bardata->{
			filterBoolean.put(bardata.getStockcode(), true);
		});
		lists =lists.parallelStream().filter(l->{
			return filterBoolean.get(l.getStocklableCode());	
		}).collect(toList());
		}
		return lists;
	}
		...
		if (groups.size() > 0) {
			...
			if (disscussInfo.size() > 0) {
				...
				groups = groups.stream().map(d -> {
					...
					Group g = groupMapper.selectByPrimaryKey(d.getGroupId());
					
					double sum = 0;
					if (d.getGroupInitMoney() != 0 && Long.parseLong(d.getImcomeRatio()) - d.getGroupInitMoney() != 0) {
						double temp = g.getGroupIncomeTotle();
